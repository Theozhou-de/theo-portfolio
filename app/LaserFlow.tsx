...
void mainImage(out vec4 fc,in vec2 frag){
  ...
  float n=fbm2(
    fuv+vec2(
      fbm2(fuv+vec2(7.3,2.1)),
      fbm2(fuv+vec2(-3.7,5.9))
    )*0.6
  );
  ...
  float xF=pow(
    clamp(1.0-smoothstep(EDGE_X0,EDGE_X1,nxE),0.0,1.0),
    EDGE_X_GAMMA
  );
...
export default function LaserFlow({
  ...
  const hexToRGB = (hex: string) => {
    ...
    const numeric = parseInt(value.slice(0, 6), 16) || 0xffffff;
    return {
  ...
  useEffect(() => {
    ...
    const syncBeamAnchor = () => {
      if (!alignToHeroFloor || !mount.parentElement) {
        uniforms.uBeamXFrac.value = horizontalBeamOffset;
        uniforms.uBeamYFrac.value = verticalBeamOffset;
        return;
      }

      const heroStyle = getComputedStyle(mount.parentElement);
      const characterX = parseFloat(
        heroStyle.getPropertyValue("--hero-character-x"),
      );
      const floorY = parseFloat(heroStyle.getPropertyValue("--hero-floor-y"));

      uniforms.uBeamXFrac.value = Number.isFinite(characterX)
        ? characterX / 100 - 0.5
        : horizontalBeamOffset;
      uniforms.uBeamYFrac.value = Number.isFinite(floorY)
        ? floorY / 100 - 0.5
        : verticalBeamOffset;
    };
    ...
    const scheduleResize = () => {
      ...
      resizeFrame = requestAnimationFrame(setSizeNow);
    };
    ...
    const updateMouse = (clientX: number, clientY: number) => {
      ...
      mouseTarget.set(
        x * pixelRatio,
        rect.height * pixelRatio - y * pixelRatio,
      );
    };
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (averageFps < lowerThreshold) {
        nextDpr = clamp(currentDprRef.current * 0.85, dprFloor, baseDpr);
      } else if (
        averageFps > upperThreshold &&
        currentDprRef.current < baseDpr
      ) {
        nextDpr = clamp(currentDprRef.current * 1.1, dprFloor, baseDpr);
      }

      if (
        Math.abs(nextDpr - currentDprRef.current) > 0.01 &&
        now - lastDprChange > 2000
      ) {
    ...
    const animate = () => {
      ...
      uniforms.uFlowTime.value =
        (uniforms.uFlowTime.value as number) + clampedDelta;
      uniforms.uFogTime.value =
        (uniforms.uFogTime.value as number) + clampedDelta;

      if (!hasFadedRef.current) {
        fade = Math.min(1, fade + clampedDelta);
